...
class HmiBackendNode(Node):
    def __init__(self):
        ...
        default_map_path = os.path.abspath('amr_delivery_ui/frontend/maps/map2.yaml')
        self.declare_parameter('map_yaml_path', os.environ.get('AMR_MAP_YAML', default_map_path))
        self.map_yaml_path = Path(os.path.expanduser(self.get_parameter('map_yaml_path').value)).resolve()

        self.control_states = {}
        self.task_command_publisher = self.create_publisher(TaskCommand, '/task/command', 10)
    ...
    def fetch_hmi_robots(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for _, row in sorted(latest.items(), key=lambda item: str(item[0])):
                ...
                robot['estopped'] = int(bool(control['estopped']) or robot['mode'] == 'PAUSED')
                robot['emergency_stopped'] = robot['estopped']
                robot['docked'] = int(bool(control['docked']) or robot['mode'] == 'DOCKED')
                robots.append(robot)
        return robots
