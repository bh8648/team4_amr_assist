...
| robot5, robot11 | `RobotStatus` | `/robot_status` | Allocation, DB | 위치·방향·배터리·작업 ID 전달 |
...
| Task | `TaskState` | `/task/state` | Allocation, DB | 재배정 차단과 DB 기록 |
...
stateDiagram-v2
    ...
    ASSIGNED --> PAUSED: HMI 정지
    FOLLOWING --> PAUSED: HMI 정지
    TRANSPORTING --> PAUSED: HMI 정지
    RETURNING --> PAUSED: HMI 정지
...
`deadlock_prevention_node`와 `DeadlockPermission` 메시지는 최신 `main`에서 제거됐다.
현재 통합본은 중앙의 거리 기반 우선순위 제어를 사용하지 않으며, 물리적 충돌 회피는
각 AMR의 Nav2 Costmap과 OAK-D·LiDAR 센서가 담당한다.
...
- Task 상태 DB UPSERT
- 웹캠 안전모 판정에 따른 FOLLOWING 로봇 경고음
- Robot bridge의 사람 앞 `0.9m` 안전거리 추종과 유실 시 Nav2 Spin 재탐색
