...
- **FOLLOWING 추종**: `/task/state`로 캐싱한 robot5의 현재 state가 `'FOLLOWING'`이 아니면 `target_person_pose` 메시지를 무시한다. `'FOLLOWING'`이고 위에서 정의한 유효성 검증을 통과한 메시지면, 올 때마다 (사람이 계속 움직이므로) 기존 Nav2 goal을 취소하고 새 goal을 보낸다 — `task_manager_node.send_navigation_goal(replace=True)`와 동일한 패턴.
...
**중요 — 실물 로봇 테스트 진행 방식**: 아래 체크리스트는 실제 로봇(TurtleBot4/Create3)이 움직이는 물리 테스트다. 시뮬레이션이 아니다. AI는 원격에서 이 단계들을 단독으로 실행·판단하지 않는다. 사용자가 로봇 옆에서 비상정지(앱 e-stop 또는 물리 버튼)를 손에 쥔 채 각 명령을 직접 트리거하고 눈으로 결과를 확인한 뒤에만 다음 단계로 진행한다. 특히 Nav2 goal 전송과 dock/undock 액션은 로봇이 실제로 주행·정렬하는 동작이므로, 첫 시도는 저속/근접 구간에서 사용자가 즉시 개입 가능한 상태로 진행한다.
  ...
  - `dock/request` True/False 각각에서 `/robot5/dock`, `/robot5/undock` 액션이 호출되는지 확인 (사용자가 도킹 스테이션 근처에서 함께 진행, 최초 시도는 충분히 근접한 상태에서 저속 확인)
