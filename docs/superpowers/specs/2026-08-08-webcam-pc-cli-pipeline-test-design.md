...
| `추종시작 [간격초=3]` | mock 좌표 10개를 간격초마다 하나씩 `/robot11/target_person_pose`로 순차 발행 시작 |
| `추종중지` | 진행 중인 순차 발행 타이머 취소 |
| `배송모드 [목적지id]` | `START_TRANSPORT` 발행. 목적지 미지정 시: `destinations`가 1개면 자동 선택, 여러 개면 목록 출력 후 재입력 요구. "배송모드"/"배송 모드" 모두 인식(공백 제거 후 비교) |
...
- `작업자감지`/`배송모드`/`배송확인` 실행 시: 캐싱된 로봇이 정확히 1개면 그 `robot_id`/`task_id`로 발행한다. 0개면 "활성 작업 없음"을 출력하고 발행하지 않는다. 2개 이상이면 로봇 지정을 요구한다(예: `작업자감지 robot11`).
...
지도 범위(`map2.yaml` 기준): `-5.31 ≤ x < 0.59`, `-5.673 ≤ y < 1.377`
...
- 이동을 유발하는 모든 CLI 명령(`추종시작`, `배송모드`, `배송확인`) — 사용자가 로봇 옆에서 비상정지를 쥔 채 직접 타이핑한다
- 도킹/언도킹 액션 트리거와 정렬 확인
- `추종시작`의 간격초 기본값(3초)은 미검증 초기값이며, 실제 주행을 보며 사용자와 함께 튜닝한다

CLI 노드 실행(`ros2 run robot_manager webcam_pc_cli`) 자체와 이동을 유발하지 않는 조회성 명령(`목적지목록`, `상태`)은 AI가 진행해도 무방하다. 그러나 위 항목들은 매 단계 사용자 확인 후 사용자가 직접 트리거한다.
