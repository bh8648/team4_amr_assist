...
> **통합 전 설계 기록:** 이 문서의 `worker_tracking_bridge_node` 및
> `target_person_pose_raw` 구조는 최종 통합본에서 사용하지 않는다. 실행 기준은
> 프로젝트 루트의 `README_INTEGRATION.md`와 현재 `robot.launch.py`이다.
